...
class Reservation:
    ...
    def __init__(self, VIN, speed, accel, enterTime, lane, t, l, w):
        ...
        self.expectedTime = int(enterTime + expect(IMC.Intersection_Manager.p1_distance, self.speed, self.accel) * 1000.0)  # time at p1
        self.expectedTime2 = int(enterTime + expect(IMC.Intersection_Manager.p2_distance, self.speed, self.accel) * 1000.0) #time at p2
    ...
    def updateZ(self,zed):
        self.updateSpeed() #update exp2 speed
        self.z = zed
        self.t1 = self.expectedTime2 + self.z
        a3 = ((self.speedLimit - self.endSpeed) / (self.z / 1000.0))
        self.OutAccel = a3 - self.accel

    #updates dv at outro entertime, speed at p2
...
class Intersection:
    ...
    inter_size = SS.Simulation_Settings.inter_size

    inter_tolerance_time = SS.Simulation_Settings.inter_tolerance_time  # intersection_side_length/[(max_Speed + min_speed)/2] (0.12s)  ---only 1 car in an in
